...
export default async function PlatformFeesPage({params}:{params:Promise<{locale:string}>}){
  ...
  const [{data,error},{data:paymentMethods,error:paymentMethodsError}]=await Promise.all([
    supabase.from('transaction_platform_fee_gates')
    ...
    profile.role==='platform_owner'
      ?supabase.from('platform_payment_methods')
  ...
  return <main className="shell premiumAppShell">
    <nav className="nav premiumNav"><a href={`/${locale}/command-center`} className="brandwrap"><div className="brand">MY CUBA CASH</div><small>{es?'Cobros y tarifas':'Collections & fees'}</small></a><div className="navlinks"><a href={`/${locale}/command-center`}>{es?'Centro de Comando':'Command Center'}</a><a href={`/${locale}/payments`}>{es?'Página de pagos':'Payments page'}</a><a href={`/${locale}/cash`}>{es?'Libro de efectivo':'Cash Ledger'}</a></div><span className="miniCta">TRUSTED REVIEW</span></nav>
    {profile.role==='platform_owner'&&<section className="section">
      {paymentMethodsError?<div className="feature premiumCard"><h3>{es?'No se pudieron cargar los métodos de cobro':'Unable to load collection methods'}</h3><p>{paymentMethodsError.message}</p></div>:<PaymentMethodSettings initialMethods={(paymentMethods??[]) as never[]} es={es}/>} 
    </section>}
    ...
    <section className="policyBlock premiumPolicy"><div><span className="eyebrow">FAIL-CLOSED</span><h2>{es?'Nunca confirmar sin evidencia':'Never confirm without evidence'}</h2></div><p>{es?'Esta acción no mueve el dinero principal de la transacción. Solo confirma que la tarifa de MY CUBA CASH fue recibida por un medio externo y verificado por un revisor autorizado.':'This action never moves transaction principal. It only confirms that the MY CUBA CASH platform fee was received through an external method and verified by an authorized reviewer.'}</p></section>
  </main>;
}
