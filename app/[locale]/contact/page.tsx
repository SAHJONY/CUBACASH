...
export default async function ContactPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell premiumAppShell" dir={locale==='ar'?'rtl':'ltr'}>
    ...
    <section className="section"><div className="sectionHead"><div><span className="eyebrow">{es?'CANALES DE CONTACTO':'CONTACT CHANNELS'}</span><h2>{es?'Teléfono y correo. Nada más.':'Phone and email. Nothing else.'}</h2></div></div>
    <div className="featureGrid">
      <article className="feature premiumCard"><span className="eyebrow">{es?'WHATSAPP BUSINESS':'WHATSAPP BUSINESS'}</span><h3>+1 281 662 8581</h3><p>{es?'Escríbenos al número publicado aquí. No usamos otros números ni otros canales.':'Message us at the number published here. We use no other numbers or channels.'}</p></article>
      <article className="feature premiumCard"><span className="eyebrow">{es?'CORREO':'EMAIL'}</span><h3>sahjonyllc@gmail.com</h3><p><a className="textLink" href="mailto:sahjonyllc@gmail.com">sahjonyllc@gmail.com</a></p><p>{es?'Correo oficial de SAHJONY LLC para consultas sobre MY CUBA CASH.':'SAHJONY LLC’s official email for MY CUBA CASH inquiries.'}</p></article>
      <article className="feature premiumCard"><span className="eyebrow">{es?'IDENTIDAD OFICIAL':'OFFICIAL IDENTITY'}</span><h3>MY CUBA CASH</h3><p>{es?'Plataforma tecnológica operada por SAHJONY LLC.':'Technology platform operated by SAHJONY LLC.'}</p></article>
      <article className="feature premiumCard"><span className="eyebrow">WEB</span><h3>www.mycubacash.com</h3><p>{es?'Usa este dominio y el WhatsApp publicado aquí para verificar que estás hablando con la plataforma correcta.':'Use this domain and the WhatsApp number published here to verify you are communicating with the correct platform.'}</p></article>
    </div></section>
