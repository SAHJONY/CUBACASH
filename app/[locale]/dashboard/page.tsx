...
const modules = [
  {k:'Agentic Command Network',d:'Multi-agent planning, research, remittance, compliance, fraud, marketplace, RFQ and reconciliation orchestration.',s:'READY'},
...
export default async function Dashboard({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={rtl?'rtl':'ltr'}>
    <nav className="nav">
      ...
      <div className="navlinks"><a href={`/${locale}`}>Home</a><a href={`/${locale}/agents`}>AI Agents</a><span>{t.market}</span><span>{t.rfq}</span><span>{t.trade}</span><a href={`/${locale}/remittances`}>Remittances</a><span>{t.compliance}</span></div>
    ...
    <footer className="footer"><strong>mycubacash.com</strong><span>Private Sector Economy Command Center</span><span>v0.4</span></footer>
  </main>;
}
