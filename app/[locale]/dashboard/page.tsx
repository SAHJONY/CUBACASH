...
export default async function Dashboard({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={rtl?'rtl':'ltr'}>
    ...
    <section className="section">
      <div className="sectionHead"><div><span className="eyebrow">FAMILY · BUSINESS · PRIVATE MARKETPLACE</span><h2>{t.dashboard}</h2></div><p>Operational control plane for the three primary mycubacash jobs, coordinated by an application brain with bounded self-healing, self-improvement and autonomous growth engines.</p></div>
      <div className="previewGrid">
        <div><span>Brain</span><strong>ORCHESTRATED</strong></div>
        <div><span>Reliability</span><strong>SELF-HEALING</strong></div>
        <div><span>Growth</span><strong>CONSENT-AWARE</strong></div>
    ...
    <section className="policyBlock"><div><span className="eyebrow">AUTONOMY PRINCIPLE</span><h2>Models propose. Policy authorizes. Evidence promotes.</h2></div><p>Safe reversible recovery can be automated. Material configuration, pricing, regulated actions, binding partner commitments and unconsented external outreach stay behind explicit policy or approval gates.</p></section>
    <footer className="footer"><strong>mycubacash.com</strong><span>Family Remittance · Business Remittance · Private-Sector Marketplace</span><span>v0.7</span></footer>
  </main>;
}
