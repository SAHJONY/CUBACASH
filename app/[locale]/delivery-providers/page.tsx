...
export default async function DeliveryProviders({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell premiumAppShell" dir={locale==='ar'?'rtl':'ltr'}>
    ...
    <section className="section">
      <div className="sectionHead"><div><span className="eyebrow">{es?'PROVEEDORES DE ENTREGA':'DELIVERY PROVIDERS'}</span><h2>{providers.length?(es?'Compara proveedores verificados':'Compare verified providers'):(es?'Estado público de la red':'Public network status')}</h2></div><p>{es?'Las tarifas las publica cada proveedor y son independientes de las tarifas de plataforma de MY CUBA CASH. La disponibilidad y el ETA siguen siendo estimados hasta que el proveedor acepte el trabajo.':'Rates are posted by each provider and are separate from MY CUBA CASH platform fees. Availability and arrival times are provider estimates until a specific job is accepted.'}</p></div>
