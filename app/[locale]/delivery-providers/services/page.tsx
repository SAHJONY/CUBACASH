...
export default function ProviderServices(){
  ...
  const groups=['FINANCIAL','FOOD','PRODUCT','HEALTH','CONNECTIVITY','TRANSPORT','HOME','PROFESSIONAL','HOSPITALITY','DELIVERY','TOPUP','COMMERCE','ERRAND','OTHER'];
  const groupLabel=(g:string)=>es?({FINANCIAL:'Remesas autorizadas',FOOD:'Alimentos',PRODUCT:'Productos',HEALTH:'Salud y bienestar',CONNECTIVITY:'Conectividad',TRANSPORT:'Transporte',HOME:'Hogar y reparaciones',PROFESSIONAL:'Servicios profesionales',HOSPITALITY:'Turismo y alojamiento',DELIVERY:'Entrega',TOPUP:'Recargas',COMMERCE:'Compras + entrega',ERRAND:'Mandados',OTHER:'Otros'} as any)[g]:({FINANCIAL:'Authorized remittance delivery',FOOD:'Food',PRODUCT:'Products',HEALTH:'Health & wellness',CONNECTIVITY:'Connectivity',TRANSPORT:'Transport',HOME:'Home & repairs',PROFESSIONAL:'Professional services',HOSPITALITY:'Hospitality',DELIVERY:'Delivery',TOPUP:'Top-ups',COMMERCE:'Commerce + delivery',ERRAND:'Errands',OTHER:'Other'} as any)[g];

  return <main className="shell premiumAppShell">
    <nav className="nav premiumNav"><a href={`/${locale}`} className="brandwrap"><div className="brand">MY CUBA CASH</div><small>{es?'Servicios locales':'Local Services'}</small></a><div className="navlinks"><a href={`/${locale}/catalog/manage`}>{es?'Mi catálogo':'My catalog'}</a><a href={`/${locale}/delivery-providers/manage`}>{es?'Tarifas de entrega':'Delivery rates'}</a><a href={`/${locale}/delivery-providers`}>{es?'Directorio público':'Public directory'}</a></div></nav>
    <section className="hero"><div className="heroCopy"><div className="eyebrow">{es?'RED DE SERVICIOS LOCALES':'LOCAL SERVICE NETWORK'}</div><h1>{es?'Convierte tu operación en un proveedor multi-servicio.':'Turn your operation into a multi-service provider.'}</h1><p className="heroLead">{es?'Publica todo lo que tu MIPYME o emprendimiento realmente ofrece: productos, alimentos, recargas, transporte, delivery, reparaciones, servicios profesionales y otras ofertas aprobadas. La entrega de efectivo de remesas usa una autorización separada.':'Publish what your private business or venture actually offers: products, food, top-ups, transport, delivery, repairs, professional services and other approved offers. Cash remittance delivery uses a separate authorization.'}</p></div></section>
    <section className="section" style={{maxWidth:1100,margin:'0 auto'}}>
      ...
      {loading?<p>{es?'Cargando servicios…':'Loading services…'}</p>:groups.map(group=>{
        ...
        return <section key={group} style={{marginBottom:34}}><h2>{groupLabel(group)}</h2><div className="featureGrid">{items.map(service=>{const d=drafts[service.code];if(!d)return null;return <article className="feature premiumCard" key={service.code}>
          ...
          {service.requires_manual_review&&<p className="sectionCopy">{es?'Este servicio requiere revisión de MY CUBA CASH antes de publicarse.':'This service requires MY CUBA CASH review before public listing.'}</p>}
      ...
      <div className="policyBlock"><div><span className="eyebrow">{es?'LÍMITE OPERACIONAL':'OPERATIONAL BOUNDARY'}</span><h2>{es?'Más servicios, sin convertir un servicio local en una actividad financiera no autorizada.':'More services without turning a local-service listing into unauthorized financial activity.'}</h2></div><p>{es?'Productos y servicios pueden publicarse según su revisión. La capacidad REMITTANCE_CASH_DELIVERY nunca se activa por registro: requiere revisión reforzada y confirmación explícita de la autorización aplicable antes de hacerse pública.':'Products and services may be offered subject to review. REMITTANCE_CASH_DELIVERY is never enabled by signup alone: it requires enhanced review and explicit confirmation of the applicable authorization before public listing.'}</p></div>
    </section>
  </main>;
}
