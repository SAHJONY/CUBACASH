...
export default function MarketplaceManage(){
  ...
  return <main className="shell">
    <nav className="nav"><a href={`/${locale}`} className="brandwrap"><div className="brand">MY CUBA CASH</div><small>{es?'Espacio de Marketplace':'Marketplace Workspace'}</small></a><div className="navlinks"><a href={`/${locale}/marketplace`}>{es?'Explorar':'Browse Marketplace'}</a><a href={`/${locale}/delivery-providers`}>{es?'Entrega':'Delivery'}</a><a href={`/${locale}/transactions`}>{es?'Transacciones':'Transactions'}</a></div></nav>
    ...
    <section className="section" style={{maxWidth:1000,margin:'0 auto'}}>
      {loading?<p>{es?'Cargando tu espacio comercial…':'Loading marketplace workspace…'}</p>:!businesses.length?<div className="feature"><h2>{es?'Necesitas un perfil de negocio':'Business profile required'}</h2><p>{es?'Necesitas un perfil comercial de MY CUBA CASH antes de publicar actividad. Sofia puede ayudarte con el onboarding.':'You need a MY CUBA CASH business profile before publishing marketplace activity. Sofia can help complete onboarding.'}</p><div className="actions"><a className="cta" href={`/${locale}/marketplace`}>{es?'Volver al Marketplace':'Back to Marketplace'}</a></div></div>:
      <form className="feature" onSubmit={submit} style={{display:'grid',gap:16}}>
        ...
        <p className="sectionCopy">{es?'No publiques teléfonos, datos bancarios, contraseñas, documentos de identidad ni credenciales de pago. El matching y la coordinación permanecen dentro de MY CUBA CASH y Sofia.':'Do not place phone numbers, bank information, passwords, identity documents or payment credentials in public offer text. Matching and transaction coordination remain inside MY CUBA CASH and Sofia.'}</p>
