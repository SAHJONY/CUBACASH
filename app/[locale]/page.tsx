...
export default async function LocaleHome({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell premiumAppShell" dir={rtl?'rtl':'ltr'}>
    ...
    <section className="section"><div className="policyBlock premiumPolicy"><div><span className="eyebrow">{es?'OPERADOR · ESTADO · CUSTODIA':'OPERATOR · STATUS · CUSTODY'}</span><h2>{es?'Operado por SAHJONY LLC · Beta v0.9 · Nunca custodian tu dinero.':'Operated by SAHJONY LLC · Beta v0.9 · We never hold your money.'}</h2></div><p>{es?'MY CUBA CASH opera como orientación de navegación y coordinación únicamente, no como transmisor de dinero. El cliente paga directamente al banco o proveedor. Cero custodia, sin excepciones. ':'MY CUBA CASH operates as navigation and coordination guidance only, not as a money transmitter. The customer pays the bank or provider directly. Zero custody, no exceptions. '}<a className="textLink" href={`/${locale}/about`}>{es?'Quiénes somos':'About us'} →</a> · <a className="textLink" href={`/${locale}/faq`}>FAQ</a></p></div></section>
