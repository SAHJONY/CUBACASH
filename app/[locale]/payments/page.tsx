...
const methods=[
  ...
  },
  {
    ...
    esBody:'Para cuentas aprobadas, MY CUBA CASH puede emitir una referencia de tarifa y aceptar evidencia de pago por un método permitido. El pago queda pendiente hasta que un revisor autorizado lo reconcilie y confirme.',
    enBody:'For approved accounts, MY CUBA CASH may issue a platform-fee reference and accept payment evidence through an allowed method. The payment remains pending until an authorized reviewer reconciles and confirms it.'
  },
  {
    key:'receipt',
    es:'Confirmación y recibo',
    en:'Confirmation and receipt',
    esBody:'La tarifa de MY CUBA CASH se registra separada del dinero principal entre participantes. Una vez confirmada, la tarifa queda marcada como pagada con su referencia y evidencia correspondiente.',
    enBody:'The MY CUBA CASH fee is recorded separately from principal exchanged between participants. Once confirmed, the fee is marked paid with its corresponding reference and evidence.'
  }
] as const;
...
export default async function PaymentsPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={locale==='ar'?'rtl':'ltr'}>
    <nav className="nav">
      <a href={`/${locale}`} className="brandwrap"><div className="brand">MY CUBA CASH</div><small>{es?'Pagos a MY CUBA CASH':'Payments to MY CUBA CASH'}</small></a>
    ...
    <section className="hero">
      <div className="heroCopy">
        <div className="eyebrow">{es?'CÓMO COBRA MY CUBA CASH':'HOW MY CUBA CASH GETS PAID'}</div>
        ...
        <p className="heroLead">{es?'En operaciones directas entre negocios, MY CUBA CASH no toma custodia del dinero principal. La aplicación calcula y registra una tarifa de plataforma separada por el servicio, la coordinación, los controles y el registro transaccional.':'In direct business-to-business transactions, MY CUBA CASH does not take custody of principal. The app calculates and records a separate platform fee for the service, coordination, controls and transaction record.'}</p>
        <p className="heroSub">{es?'Nunca envíes dinero usando datos bancarios o instrucciones recibidas fuera de la aplicación o de un canal autorizado de MY CUBA CASH. Usa siempre la referencia de tarifa mostrada para tu operación.':'Never send money using banking details or instructions received outside the app or an authorized MY CUBA CASH channel. Always use the platform-fee reference shown for your transaction.'}</p>
    ...
    <section className="section">
      <div className="sectionHead"><div><span className="eyebrow">{es?'MÉTODOS':'METHODS'}</span><h2>{es?'Solo métodos presentados por la aplicación o autorizados por MY CUBA CASH.':'Only methods presented by the app or authorized by MY CUBA CASH.'}</h2></div></div>
