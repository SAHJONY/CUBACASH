...
export default async function Remittances({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={rtl?'rtl':'ltr'}>
    ...
    <section className="section" id="delivery">
      <div className="sectionHead"><div><span className="eyebrow">{es?'PROVEEDORES DE ENTREGA':'DELIVERY PROVIDERS'}</span><h2>{es?'Personas a tiempo parcial y negocios privados':'Part-time individuals and private businesses'}</h2></div><p>{es?'MIPYMEs, emprendedores y otros actores privados verificados pueden ofrecer entrega, productos y servicios a las familias. Cuando un proveedor también deba entregar efectivo de una remesa, esa capacidad requiere verificación y autorización independiente antes de habilitarse.':'Verified private businesses, entrepreneurs and other private-sector providers may offer delivery, products and services to families. When a provider must also deliver remittance cash, that capability requires separate verification and authorization before it is enabled.'}</p></div>
