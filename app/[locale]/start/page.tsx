  ...
  return <main className="shell premiumAppShell">
    ...
    <section className="section" style={{maxWidth:980,margin:'0 auto'}}>
      ...
      <form onSubmit={submit} className="feature" style={{display:'grid',gap:14}}><h2>{es?'2. Remitente y monto':'2. Sender and amount'}</h2>
        ...
        {fee!==null&&<p><strong>{es?'Tarifa estimada de plataforma familiar MY CUBA CASH:':'Estimated MY CUBA CASH family platform fee:'}</strong> ${fee.toFixed(2)} USD. {es?'Cargos separados de settlement, FX o entrega pueden aplicar y deben mostrarse antes del compromiso.':'Separate settlement, FX or delivery charges may apply and must be disclosed before commitment.'}</p>}
