...
export default async function TermsPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={locale==='ar'?'rtl':'ltr'}>
    <nav className="nav"><a href={`/${locale}`} className="brandwrap"><div className="brand">mycubacash.com</div><small>Terms</small></a><div className="navlinks"><a href={`/${locale}/privacy`}>Privacy</a><a href={`/${locale}/fees`}>Fees</a><a href={`/${locale}/auth`}>Account</a></div></nav>
    <section className="section" style={{maxWidth:900,margin:'0 auto'}}><span className="eyebrow">CUSTOMER TERMS</span><h1>Rules for using mycubacash</h1>
      <div className="feature" style={{display:'grid',gap:14}}>
        <p>Use mycubacash only for legitimate family support, private-business payments, marketplace activity and delivery coordination. Information submitted must be accurate and belong to transactions you are authorized to initiate.</p>
