...
export default async function TransactionsPage({params}:{params:Promise<{locale:string}>}){
  ...
  return <main className="shell" dir={locale==='ar'?'rtl':'ltr'}>
    <nav className="nav"><a href={`/${locale}`} className="brandwrap"><div className="brand">MY CUBA CASH</div><small>My Transactions</small></a><div className="navlinks"><a href={`/${locale}/start`}>Start Transaction</a><a href={`/${locale}/fees`}>Fees</a><a href={`/${locale}/privacy`}>Privacy</a></div></nav>
    <section className="hero"><div className="heroCopy"><div className="eyebrow">TRANSACTION TRACKING</div><h1>Know what is happening and what to do next.</h1><p className="heroLead">Each request has a permanent MY CUBA CASH reference, customer-readable status and a detail page for submitting supporting information.</p></div></section>
    <section className="section">
      {error?<p>Unable to load your transactions right now.</p>:!data?.length?<div className="feature"><h2>No transactions yet</h2><p>Create your first family support request to receive a MY CUBA CASH transaction reference.</p><div className="actions"><a className="cta" href={`/${locale}/start`}>Start Transaction</a></div></div>:
